...
function setClaim(id,label,mode){document.querySelector(`input[name="claim_${id}"][value="${label}"]`).checked=true;let input=document.getElementById('interval_'+id);if(mode==='all')input.value=`0-${task.frame_count-1}`;else if(mode==='clear')input.value='';else if(mode==='suggested')input.value=input.dataset.suggested;else if(mode==='boxed')input.value=input.dataset.boxed;refreshEvidence(id)}
