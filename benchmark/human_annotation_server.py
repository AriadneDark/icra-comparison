...
try:
    from .hybrid_common import (
        METHODS, ROLE_ORDER, build_candidates, intervals_from_frames, load_json,
        load_method_output, load_ontology, resolve_study_path, scoped_artifact, write_json,
    )
except ImportError:
    from hybrid_common import (
        METHODS, ROLE_ORDER, build_candidates, intervals_from_frames, load_json,
...
function intervalsFromSet(values){let xs=[...values].sort((a,b)=>a-b),out=[];for(const x of xs){let last=out[out.length-1];if(!last||x>last[1]+1)out.push([x,x]);else last[1]=x}return out}
function refreshEvidence(id){let input=document.getElementById('interval_'+id);if(!input)return;let included;try{included=intervalSet(input.value)}catch{return}for(const figure of document.querySelectorAll(`figure[data-claim="${id}"]`)){figure.classList.toggle('excluded',!included.has(Number(figure.dataset.frame)))}}
function toggleFrame(id,frame){let input=document.getElementById('interval_'+id),values=intervalSet(input.value);values.has(frame)?values.delete(frame):values.add(frame);input.value=formatIntervals(intervalsFromSet(values));document.querySelector(`input[name="claim_${id}"][value="yes"]`).checked=true;refreshEvidence(id)}
function setClaim(id,label,mode){document.querySelector(`input[name="claim_${id}"][value="${label}"]`).checked=true;let input=document.getElementById('interval_'+id);if(mode==='all')input.value=`0-${task.frame_count-1}`;else if(mode==='clear')input.value='';else if(mode==='suggested')input.value=input.dataset.suggested;refreshEvidence(id)}
...
class AnnotationApp:
    ...
    def vlm_suggestions(self, record: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
        path = self.study_root / "vlm" / f"{record['video_id']}.json"
        if not path.exists():
            return {}, {}
        ...
        for verdict in payload.get("verdicts", []):
            ...
            if label not in {"yes", "no", "uncertain"}:
                continue
            positive_frames = [
                int(index) for index, value in verdict.get("frame_verdicts", {}).items()
                if value == "yes"
            ]
            claims[verdict["claim_id"]] = {
                "label": label, "intervals": intervals_from_frames(positive_frames),
            }
        role_suggestions: dict[str, Any] = {}
        for role in payload.get("proposal", {}).get("roles", []):
            name, label = role.get("role"), str(role.get("label") or "").strip()
            if name in ROLE_ORDER and label and name not in role_suggestions:
                role_suggestions[name] = {"label": label}
        return claims, role_suggestions
    ...
    def task(self, index: int) -> dict[str, Any]:
        ...
        vlm_suggestions, role_suggestions = self.vlm_suggestions(record)
        return {
            ...
            "vlm_suggestions": vlm_suggestions, "role_suggestions": role_suggestions,
        }
