...
`--overwrite` уже готовые JSON текущей схемы пропускаются. Старые результаты,
где VLM пыталась восстанавливать интервалы по 10 кадрам, пересчитаются
автоматически.
...
Важно: если в подготовленном эпизоде 30 кадров, эти 10 кадров являются 10
независимыми checkpoint-ами. Оставшиеся 20 не считаются ни положительными, ни
отрицательными. Поэтому VLM/hybrid precision и recall относятся к выбранным
checkpoint-ам, а точность полных временных интервалов оценивается на ручной
разметке.
